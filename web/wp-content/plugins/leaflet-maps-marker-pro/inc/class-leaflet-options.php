<?php
/**
 * Settings class - Maps Marker Pro
...
class Class_leaflet_options {
	...
	public function display_page() {
		...
		echo '<h1 style="padding-top:15px;float:left;">'.__('Settings','lmm').'</h1><div class="wrap lmmsettings" style="clear:both;">';
        ...
        foreach($this->panes as $pane_slug => $pane){
	        ...
	        $sub_panel_index = 0;
                foreach($sections as $slug => $section){
					...
					echo "<h2 class='h2-lmm-settings'>".strip_tags($this->sections[$section])."</h2>"; //info: strip_tags for removing <br/> dividers
	...
	public function get_settings() {
		...
		*/
		...
		$this->_settings['defaults_marker_shortcode_helptext'] = array(
			...
			'section' => 'mapdefaults-marker_map_defaults',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Default values for markers added directly','lmm') . '</h3>',
			'desc'    => __( 'You can also add markers directly to posts or pages without having to save them to your database previously. You just have to use the shortcode with the attributes lat and lon (e.g. <strong>[mapsmarker lat="48.216038" lon="16.378984"]</strong>).', 'lmm') . ' ' . sprintf(__('For more information please visit the Shortcode API docs at %1$s','lmm'), '<a href="https://www.mapsmarker.com/shortcode-api" target="_blank">https://www.mapsmarker.com/shortcode-api</a>') . '<br/><br/><img src="'. LEAFLET_PLUGIN_URL .'inc/img/help-marker-direct.jpg" width="408" height="80" /><br/><br/>' . __('Defaults values for markers added directly:','lmm'),
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['defaults_marker_icon_title'] = array(
			...
			'desc'    => __('Show marker name for the browser tooltip that appear on marker hover (tooltip is always hidden if marker name is empty).','lmm') . '<br/>' . sprintf(__('This setting has no effect if marker tooltips available since v3.0 are enabled (see related section %1$s).','lmm'), '"' . __('Marker tooltip settings','lmm') . '"'),
			'type'    => 'radio',
			'std'     => 'show',
			'choices' => array(
				'show' => __('show','lmm'),
		...
		*/
		...
		$this->_settings['defaults_marker_popups_rise_on_hover'] = array(
			...
			'pane'    => 'mapdefaults',
			'section' => 'mapdefaults-marker_popups',
			'title'   => __('Open popups on mouse hover instead of mouse click','lmm') . $pro_option,
		...
		*/
		...
		$this->_settings['clustering_helptext2'] = array(
			...
			'desc'    => sprintf(__('Options to pass when creating the L.Polygon for styling (<a href="%1s" target="_blank">more details</a>)','lmm'), 'http://leafletjs.com/reference-1.0.3.html#path-stroke') . '<br/><img src="'. LEAFLET_PLUGIN_URL .'inc/img/help-clustering-polygon-options.jpg" width="141" height="91" />',
		...
		$this->_settings['clustering_helptext4'] = array(
			...
			'desc'    => sprintf(__('Allows you to specify <a href="%1$s" target="_blank">PolylineOptions</a> to style spider legs.','lmm'), 'http://leafletjs.com/reference-1.0.3.html#polyline-smoothfactor'),
			'type'    => 'helptext-twocolumn'
		);
		...
		*/
		...
		$this->_settings['map_keyboard_navigation_options_helptext'] = array(
			...
			'section' => 'mapdefaults-interaction',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Keyboard navigation options','lmm') . '</h3>',
			'desc'    => __( 'The following settings will be used for all marker and layer maps', 'lmm'),
			'type'    => 'helptext'
		);
		...
		$this->_settings['map_keyboard_navigation_options_keyboardpandelta'] = array(
			'version' => 'p3.0',
			'pane'    => 'mapdefaults',
			'section' => 'mapdefaults-interaction',
			'title'   => 'keyboardPanDelta',
		...
		$this->_settings['map_panning_inertia_options_helptext'] = array(
			...
			'section' => 'mapdefaults-interaction',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Panning inertia options','lmm') . '</h3>',
			'desc'    => __( 'The following settings will be used for all marker and layer maps', 'lmm'),
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['map_fullscreen_button'] = array(
			...
			'desc'    => __('Whether to add a button for displaying maps in fullscreen via HTML5','lmm') . '<br/><img src="'. LEAFLET_PLUGIN_URL .'inc/img/help-fullscreen.png" width="87" height="99" />',
			'type'    => 'radio',
			'std'     => 'true',
			'choices' => array(
				'true' => __('true','lmm'),
		...
		$this->_settings['map_scale_control_helptext'] = array(
			...
			'section' => 'mapdefaults-control',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Scale control','lmm') . '</h3>',
		...
		*/
		...
		$this->_settings['geolocate_status'] = array(
			...
			'desc'    => sprintf(__('Recommendation: only enable this feature if your maps are accessible via https, which is required by the following browsers: %1$s. For more details, <a href="%2$s" target="_blank">please click here</a>','lmm'), 'Chrome 50+, Safari 10+, Firefox 55+', 'https://www.mapsmarker.com/geolocation-https-only'),
			'type'    => 'radio',
			'std'     => 'false',
			'choices' => array(
				'true' => __('true','lmm'),
		...
		$this->_settings['geolocate_clickBehavior_inView'] = array(
			...
			'pane'    => 'mapdefaults',
			'section' => 'mapdefaults-geolocate',
			'title'   => 'clickBehavior inView' . $pro_option,
		...
		$this->_settings['geolocate_clickBehavior_outOfView'] = array(
			...
			'pane'    => 'mapdefaults',
			'section' => 'mapdefaults-geolocate',
			'title'   => 'clickBehavior outOfView' . $pro_option,
		...
		$this->_settings['geolocate_circleStyle'] = array(
			...
			'desc'    => sprintf(esc_attr__('change the style of the circle around the location of the user, example: %1$s','lmm'), "<br/>'color':'#ff0000','fillColor':'#136AEC','fillOpacity':'0.15','weight':'2','opacity':'0.5'"),
			'std'     => '',
			'type'    => 'text'
		);
		$this->_settings['geolocate_markerStyle'] = array(
			...
			'desc'    => sprintf(esc_attr__('change the style of the marker of the location of the user, example: %1$s','lmm'), "<br/>'color':'#ff0000','fillColor':'#136AEC','fillOpacity':'0.15','weight':'2','opacity':'0.5','radius':'5'"),
			'std'     => '',
			'type'    => 'text'
		);
		$this->_settings['geolocate_followCircleStyle'] = array(
			...
			'desc'    => sprintf(esc_attr__('change the style of the circle around the location of the user while following, example: %1$s','lmm'), "<br/>'color':'#ff0000','fillColor':'#136AEC','fillOpacity':'0.15','weight':'2','opacity':'0.5'"),
			'std'     => '',
			'type'    => 'text'
		);
		$this->_settings['geolocate_followMarkerStyle'] = array(
			...
			'desc'    => sprintf(esc_attr__('change the style of the marker of the location of the user while following, example: %1$s','lmm'), "<br/>'color':'#ff0000','fillColor':'#136AEC','fillOpacity':'0.15','weight':'4','opacity':'0.5','radius':'5'"),
			'std'     => '',
			'type'    => 'text'
		);
		$this->_settings['geolocate_locateOptions'] = array(
			...
			'desc'    => sprintf(__('define additional location options e.g enableHighAccuracy: true, maxZoom: 10<br/>reference: %1$s','lmm'), '<a href="http://leafletjs.com/reference-1.0.3.html#locate-options-watch" target="_blank">http://leafletjs.com/reference-1.0.3.html#locate-options-watch</a>'),
			'std'     => 'watch: true',
			'type'    => 'text'
		);
		...
		*/
		...
		$this->_settings['qrcode_google_helptext'] = array(
			...
			'section' => 'mapdefaults-qr_code',
			'std'     => '',
			'title'   => '<img src="' . LEAFLET_PLUGIN_URL . 'inc/img/help-google-qr.png" width="122" height="126" />',
       ...
       $this->_settings['marker_tooltip_status'] = array(
            ...
            'pane'    => 'mapdefaults',
            'section' => 'mapdefaults-marker_tooltip',
            'title'   => __('Marker tooltip status','lmm') . $pro_option,
            'desc'    => '',
            'type'    => 'radio',
            'std'     => 'enabled',
            'choices' => array(
		...
		$this->_settings['marker_tooltip_direction'] = array(
			...
			'pane'    => 'mapdefaults',
			'section' => 'mapdefaults-marker_tooltip',
			'title'   => __('Direction','lmm') . $pro_option,
        ...
        $this->_settings['marker_tooltip_offset_x'] = array(
            ...
            'pane'    => 'mapdefaults',
            'section' => 'mapdefaults-marker_tooltip',
            'title'   => __( 'Offset (x)', 'lmm' ) . $pro_option,
        ...
        $this->_settings['marker_tooltip_offset_y'] = array(
            ...
            'pane'    => 'mapdefaults',
            'section' => 'mapdefaults-marker_tooltip',
            'title'   => __( 'Offset (y)', 'lmm' ) . $pro_option,
            'desc'    => __('Optional offset value of the tooltip position.','lmm'),
            'std'     => '0',
            'type'    => 'text'
        );
        ...
        $this->_settings['marker_tooltip_opacity'] = array(
            ...
            'pane'    => 'mapdefaults',
            'section' => 'mapdefaults-marker_tooltip',
            'title'   => __( 'Opacity', 'lmm' ) . $pro_option,
		...
		*/
		...
		$this->_settings['geocoding_provider_helptext2'] = array(
			...
			'section' => 'geocoding-general_settings',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Rate limit savings and performance settings','lmm') . '</h3>',
			'desc'    => '',
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['geocoding_algolia_helptext2'] = array(
			...
			'section' => 'geocoding-algolia',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Authentication settings','lmm') . '</h3>',
		...
		$this->_settings['geocoding_algolia_helptext3'] = array(
			...
			'section' => 'geocoding-algolia',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Advanced settings','lmm') . '</h3>',
			'desc'    => '',
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['geocoding_photon_helptext2'] = array(
			...
			'section' => 'geocoding-photon',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Location Biasing','lmm') . '</h3>',
		...
		$this->_settings['geocoding_photon_helptext3'] = array(
			...
			'section' => 'geocoding-photon',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Filter results by tags and values','lmm') . '</h3>',
			'desc'    => '',
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['geocoding_mapzen_search_helptext3'] = array(
			...
			'section' => 'geocoding-mapzen',
			'std'     => '',
			'title'   => '<h2 class="h3-lmm-settings">' . __('Advanced settings','lmm') . '</h2><h3 class="h3-lmm-settings">' . __('Local focus point','lmm') . '</h3>',
			'desc'    => __('To focus your search based upon a geographical area, please provide latitude and longitude values.','lmm'),
			'type'    => 'helptext'
		);
		...
		$this->_settings['geocoding_mapzen_search_helptext4'] = array(
			...
			'section' => 'geocoding-mapzen',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Search within a particular country','lmm') . '</h3>',
			'desc'    => '',
			'type'    => 'helptext'
		);
		...
		$this->_settings['geocoding_mapzen_search_helptext5'] = array(
			...
			'section' => 'geocoding-mapzen',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Narrow search','lmm') . '</h3>',
		...
		$this->_settings['geocoding_mapzen_search_helptext6'] = array(
			...
			'section' => 'geocoding-mapzen',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Sources','lmm') . '</h3>',
		...
		$this->_settings['geocoding_mapzen_search_helptext7'] = array(
			...
			'section' => 'geocoding-mapzen',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Layers','lmm') . '</h3>',
        ...
        */
		...
		$this->_settings['geocoding_mapquest_geocoding_helptext2'] = array(
			...
			'std'     => '',
			'title'   => '',
			'desc'    => '<h3 class="h3-lmm-settings">' . __('Location Biasing','lmm') . '</h3>',
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['geocoding_google_geocoding_helptext2'] = array(
			...
			'std'     => '',
			'title'   => '',
			'desc'    => '<h3 class="h3-lmm-settings">' . __('Authentication for Google Maps APIs Premium Plan customers','lmm') . '</h3><br/>' . sprintf(__('If you are a <a href="%1$s" target="_blank">Google Maps APIs Premium Plan</a> customer, please change the authentication method above to %2$s and fill in the credentials below which you received in the welcome email for "Google Maps APIs Premium Plan"-customers from Google.','lmm'), 'https://developers.google.com/maps/premium/overview', '"client ID + signature"'),
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['edgeBufferTiles'] = array(
			...
			'pane'    => 'basemaps',
			'section' => 'basemaps-basemap_global_settings',
			'title'   => 'edgeBufferTiles' . $pro_option,
		...
		$this->_settings['basemaps_nowrap_enabled'] = array(
			...
			'pane'    => 'basemaps',
			'section' => 'basemaps-basemap_global_settings',
			'title'   => __('Enable nowrap?','lmm') . $pro_option,
		...
		*/
		...
		$this->_settings['stamen_terrain_helptext'] = array(
			...
			'section' => 'basemaps-stamen_maps',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Stamen Terrain map settings','lmm') . '</h3>',
		...
		$this->_settings['stamen_toner_helptext'] = array(
			...
			'section' => 'basemaps-stamen_maps',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Stamen Toner map settings','lmm') . '</h3>',
		...
		$this->_settings['stamen_watercolor_helptext'] = array(
			...
			'section' => 'basemaps-stamen_maps',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Stamen Watercolor map','lmm') . '</h3>',
		...
		*/
		...
		$this->_settings['google_maps_api_status'] = array(
			...
			'pane'    => 'basemaps',
			'section' => 'basemaps-google_js_api',
			'title'   => __('Google Maps JavaScript API','lmm'),
		...
		$this->_settings['google_maps_api_key'] = array(
			...
			'pane'    => 'basemaps',
			'section' => 'basemaps-google_js_api',
			'title'   => __( 'Google Maps JavaScript API key', 'lmm'),
		...
		$this->_settings['google_maps_plugin'] = array(
			...
			'pane'    => 'basemaps',
			'section' => 'basemaps-google_js_api',
			'title'   => __('Google Maps leaflet implementation','lmm') . $pro_option,
			'desc'    => sprintf(__('GoogleMutant plugin is recommended although not supported on Internet Explorer 10 or lower and several older browsers versions (maps will automatically switch to OpenStreetMap for those users).<br/>Current browser market share for affected browsers: about %1$s and declining steadily. If you do not want Google basemaps to automatically switch to OpenStreetMap for those outdated browsers, please activate the legacy plugin.','lmm'), '2% (05/2017)'),
			'type'    => 'radio',
			'std'     => 'google_mutant',
			'choices' => array(
		...
		*/
		...
		$this->_settings['bingmaps_culture_helptext'] = array(
			...
			'section' => 'basemaps-bing',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Bing Culture Parameter','lmm') . '</h3>',
		...
		*/
		$this->_settings['mapbox_helptext'] = array(
			...
			'std'     => '',
			'title'   => '',
			'desc'    => sprintf(__('A tutorial on how to configure Mapbox basemaps can be found at %1$s','lmm'), '<a href="https://www.mapsmarker.com/mapbox" target="_blank">https://www.mapsmarker.com/mapbox</a>'),
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['misc_general_helptext2'] = array(
			...
			'std'     => '',
			'title'   => '',
			'desc'    => '<h3 class="h3-lmm-settings">' . __('Whitelabel settings','lmm') . '</h3>',
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['misc_marker_listing_columns_helptext2'] = array(
			...
			'std'     => '',
			'title'   => '',
			'desc'    => '<h3 class="h3-lmm-settings">' . __('Available columns','lmm') . '</h3><br/>' . __( 'Please select the columns which should be available on the page "List all markers"', 'lmm'),
			'type'    => 'helptext'
		);
		...
		$this->_settings['misc_marker_listing_sort_helptext'] = array(
			...
			'section' => 'misc-list_all_markers',
			'std'     => '',
			'title'   => '<h3 class="h3-lmm-settings">' . __('Sort order','lmm') . '</h3>',
		...
		*/
		...
		$this->_settings['misc_layer_listing_columns_helptext2'] = array(
			...
			'std'     => '',
			'title'   => '',
			'desc'    => '<h3 class="h3-lmm-settings">' . __('Available columns','lmm') . '</h3><br/>' . __( 'Please select the columns which should be available on the page "List all layers"', 'lmm'),
			'type'    => 'helptext'
		);
		...
		*/
		...
		$this->_settings['capabilities_view_others'] = array(
			...
			'pane'    => 'misc',
			'section' => 'misc-permissions',
			'title'   => __( 'User role needed for viewing markers/layers from other users (applies only to backend access)', 'lmm' ) . $pro_option,
			'desc'    => '',
			'type'    => 'radio',
			'std'     => 'edit_posts',
			'choices' => array(
		...
		*/
		...
		$this->_settings['rewrite_baseurl'] = array(
			...
			'pane'    => 'misc',
			'section' => 'misc-compatibility',
			'title'   => __('Permalinks base URL','lmm') . $pro_option,
		...
		*/
		...
		$this->_settings['multilingual_integration_status'] = array(
			...
			'pane'    => 'misc',
			'section' => 'misc-wordpress_integration',
			'title'   => __('WPML/Polylang integration','lmm') . $pro_option,
		...
		$this->_settings['rewrite_slug'] = array(
			...
			'pane'    => 'misc',
			'section' => 'misc-wordpress_integration',
			'title'   => __('Permalinks slug','lmm') . $pro_option,
			'desc'    => sprintf(__('Used to create pretty links to fullscreen maps or API endpoints. Default value: %1$s','lmm'), '<strong>maps</strong>') . '<br/>' . sprintf(__('Example link to fullscreen marker map ID 1: %1$s','lmm'), MMP_Rewrite::get_base_url() . '<strong>maps</strong>/fullscreen/marker/1/'),
			'type'    => 'text',
			'std'     => 'maps'
		);
	...
	*/
	...
	public function register_settings() {
		...
		flush_rewrite_rules(true); //info: needed for MMP Rewrite / slug for endpoints update
	}
	/**
	...
	public function save_defaults_for_new_options() {
		...
		if (version_compare(get_option('leafletmapsmarker_version'),'3.11','='))
		{
			...
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == '3.11.1')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
		...
		if (version_compare(get_option('leafletmapsmarker_version'),'3.12','='))
		{
			...
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == '3.12.1')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
		...
		if (version_compare(get_option('leafletmapsmarker_version'),'3.12.1','='))
		{
			...
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == '3.13')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
		...
		*/
		...
		//info:  set defaults for options introduced in pro version 2.9
		if (version_compare(get_option('leafletmapsmarker_version_pro'),'2.8.1','='))
		{
			$new_options_defaults = array();
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == 'p2.9')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
		...
		//info:  set defaults for options introduced in pro version 3.0
		if (version_compare(get_option('leafletmapsmarker_version_pro'),'2.9','='))
		{
			$new_options_defaults = array();
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == 'p3.0')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
		...
		//info:  set defaults for options introduced in pro version 3.0.1
		if (version_compare(get_option('leafletmapsmarker_version_pro'),'3.0','='))
		{
			$new_options_defaults = array();
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == 'p3.0.1')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
		...
		//info:  set defaults for options introduced in pro version 3.1
		if (version_compare(get_option('leafletmapsmarker_version_pro'),'3.0.1','='))
		{
			$new_options_defaults = array();
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == 'p3.1')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
		...
		//info:  set defaults for options introduced in pro version 3.2
		if (version_compare(get_option('leafletmapsmarker_version_pro'),'3.1','='))
		{
			$new_options_defaults = array();
			foreach ( $this->settings as $id => $setting )
			{
				if ( $setting['type'] != 'heading' && $setting['type'] != 'helptext' && $setting['type'] != 'helptext-twocolumn' && $setting['version'] == 'p3.2')
				{
				$new_options_defaults[$id] = $setting['std'];
				}
			}
