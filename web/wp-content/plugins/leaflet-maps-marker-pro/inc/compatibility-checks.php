...
if (is_plugin_active('wp-google-analytics/wp-google-analytics.php') ) {
	echo '<p><div class="notice notice-error" style="padding:10px;"><strong>' . sprintf(__('Warning: you are using the outdated plugin WP Google Analytics which is incompatible with %1$s. Please update to a more current Google analytics plugin like %2$s','lmm'), 'Maps Marker Pro', '<a href="https://wordpress.org/extend/plugins/google-analytics-for-wordpress/" target="_blank">Google Analytics by MonsterInsights</a>') . '</strong></div></p>';
}
...
if (is_plugin_active('wp-minify/wp-minify.php') ) {
	...
	if ($lmm_bwpminify_options['enable_html'] == '1') {
			echo '<p><div class="notice notice-error" style="padding:10px;"><strong>' . sprintf(__('Warning: you are using the plugin "WP Minify" which is causing %1$s layer maps to break as the option "Enable HTML Minification" is active. Please disable this option under Settings / WP Minify.','lmm'), 'Maps Marker Pro') . '</strong></div></p>';
	}
}
...
if (is_plugin_active('wp-external-links/wp-external-links.php') ) {
	echo '<p><div class="notice notice-error" style="padding:10px;">' . sprintf(__('Warning: you are using the plugin %1$s which can cause layer maps to break - please deactivate the plugin %1$s in order to display maps created with %2$s properly!','lmm'), '"WP External Links"', '"Maps Marker Pro"' ) . '</div></p>';
}
...
//info: WPML update strings for MMP 2.9 users only for MMP 3.0+ compatibility #337
if (is_plugin_active('sitepress-multilingual-cms/sitepress.php') ) {
	if (version_compare(get_option('leafletmapsmarker_version_pro_before_update'),'2.9','=')) { //info: only run SQL for affected users
		...
		if ($results != NULL) {
			...
			echo '<p><div id="wpml_upgrade_info" class="notice notice-warning" style="padding:10px;"><strong>' . __('Warning: your WPML string translations for Maps Marker Pro need to be updated!','lmm') . '</strong><br/>' . sprintf(__('Making a database backup of the affected tables %1$s and %2$s is not required but recommended in case there are any unexpected issues.','lmm'), '<code>' . $wpdb->prefix . 'icl_strings</code>', '<code>' . $wpdb->prefix . 'icl_string_translations</code>') . '<br/>' . sprintf(__('<a href="%1$s">Please click here to start the automatic update process</a>','lmm'), LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_tools&action=wpml_upgrade&_wpnonce=' . $toolnonce) . '</div></p>';
		}
	}
}
...
//info: permalink compatibility check
if ($wp_rewrite->using_mod_rewrite_permalinks() && wp_remote_retrieve_response_code(wp_remote_head(MMP_Globals::translate_permalink(MMP_Rewrite::get_base_url() . MMP_Rewrite::get_slug() . '/geojson/layer/0/'))) == 404) {
	if (wp_remote_retrieve_response_code(wp_remote_head(MMP_Globals::translate_permalink(MMP_Rewrite::get_base_url() . 'wp/' . MMP_Rewrite::get_slug() . '/geojson/layer/0/'))) == 404) {
		echo '<p><div class="notice notice-error" style="padding:10px;">' . sprintf(__('Warning: the permalinks for the Maps Marker Pro API endpoints are not working correctly which means markers cannot be displayed and API links will not work.<br/>To fix this, please navigate to <a href="%1$s">Compatibility Settings</a> and add <strong>%2$s</strong> folder to the option "Permalinks base URL".','lmm'), LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_settings#lmm-misc-compatibility', MMP_Globals::translate_permalink(MMP_Rewrite::get_base_url() . 'wp/' . MMP_Rewrite::get_slug() . '/geojson/layer/0/')) . '</div></p>';
	} else {
		echo '<p><div class="notice notice-error" style="padding:10px;">' . sprintf(__('Warning: the permalinks for the Maps Marker Pro API endpoints are not working correctly which means markers cannot be displayed and API links will not work.<br/>To fix this, please navigate to <a href="%1$s">Compatibility Settings</a> and add the URL to your WordPress folder to the option "Permalinks base URL".','lmm'), LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_settings#lmm-misc-compatibility') . '</div></p>';
	}
}
