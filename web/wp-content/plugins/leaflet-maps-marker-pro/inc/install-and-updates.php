...
if (version_compare(get_option('leafletmapsmarker_version_pro'),'3.1','=')) {
	...
	if ( $version_before_update === FALSE ) {
		set_transient( 'leafletmapsmarker_version_pro_before_update', 'MapsMarkerPro-transient-for-dynamic-changelog', 60 );
		update_option('leafletmapsmarker_version_pro_before_update', '3.1');
	}
	update_option('leafletmapsmarker_version_pro', '3.1.1');
	...
	set_transient( 'leafletmapsmarker_install_update_cache_vp311', 'execute install and update-routine only once a day', 60*60*24 );

	/*remove Visualead qr cache directory */
	$lmm_upload_dir = wp_upload_dir();
