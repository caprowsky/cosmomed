...
var MMP_Geocoding = function( provider , options, is_edit, typinginterval, min_chars_start_search_as_you_type){
	...
	this.providers = {
		...
		photon: {
			...
			format: function(options, text){
				...
				var housenumber = options.properties.housenumber;
				var street = options.properties.street;
				...
				var address = (street ? ', ' + street + (housenumber ? ' ' + housenumber : '') : '') + (state ? ', ' + state : '') + (country ? ', ' + country : '');
		...
		google_geocoding: {
			...
			get_url: function(){
				var url = lmm_ajax_vars.lmm_ajax_site_url + lmm_ajax_vars.lmm_ajax_lmm_slug + '/google-places/autocomplete/?mmp_address={query}';
				            ...
				            var ajaxObj = jQuery.ajax(
				              {
				                ...
				                beforeSend: function(request){
					            	...
					            	//info: save the address old value (to automatically trigger new search if fallback geocoding provider is used)
	...
	*/
	...
	this.google_places_selected = function( place_id ){
		var details_endpoint = lmm_ajax_vars.lmm_ajax_site_url + lmm_ajax_vars.lmm_ajax_lmm_slug + '/google-places/details/?mmp_place_id={place_id}&_wpnonce={wpnonce}';
		jQuery.ajax(
          {
