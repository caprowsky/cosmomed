<?php
if ( !class_exists('MMPPuc_v4p1_Plugin_UpdateChecker', false) ):
	 ...
	 * @access public
	 */
	class MMPPuc_v4p1_Plugin_UpdateChecker extends MMPPuc_v4p1_UpdateChecker {
		 ...
		 * @return MMPPuc_v4p1_Scheduler
		 */
		protected function createScheduler($checkPeriod) {
			$scheduler = new MMPPuc_v4p1_Scheduler($this, $checkPeriod, array('load-plugins.php'));
		 ...
		 * @return MMPPuc_v4p1_Plugin_Info
		 */
		public function requestInfo($queryArgs = array()) {
			list($pluginInfo, $result) = $this->requestMetadata('MMPPuc_v4p1_Plugin_Info', 'request_info', $queryArgs);

			if ( $pluginInfo !== null ) {
				/** @var MMPPuc_v4p1_Plugin_Info $pluginInfo */
		 ...
		 *
		 * @return MMPPuc_v4p1_Update|null An instance of Plugin_Update, or NULL when no updates are available.
		 */
		public function requestUpdate() {
			...
			$update = MMPPuc_v4p1_Plugin_Update::fromPluginInfo($pluginInfo);
		 ...
		 *
		 * @return MMPPuc_v4p1_Plugin_Update|null
		 */
		public function getUpdate() {
			$update = parent::getUpdate();
			if ( isset($update) ) {
				/** @var MMPPuc_v4p1_Plugin_Update $update */
		...
		public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
			...
			if ( $isRelevant && $this->userCanInstallUpdates() ) {
				...
				$linkText = apply_filters(
					...
					//__('Manually check for updates', 'lmm')
					__('', 'lmm')
				);
				if ( !empty($linkText) ) {
		...
		public function displayManualCheckResult() {
			if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
				...
				if ( $status == 'no_update' ) {
					$message = sprintf(_x('The %s plugin is up to date.', 'the plugin title', 'lmm'), $title);
				} else if ( $status == 'update_available' ) {
					$message = sprintf(_x('A new version of the %s plugin is available.', 'the plugin title', 'lmm'), $title);
				} else {
					$message = sprintf(__('Unknown update checker status "%s"', 'lmm'), htmlentities($status));
				}
				printf(
		...
		protected function createDebugBarExtension() {
			return new MMPPuc_v4p1_DebugBar_PluginExtension($this);
		}
	}
