...
if (!lmm_is_plugin_active('leaflet-maps-marker-pro/leaflet-maps-marker.php') ) {
	...
	if ($file_url === NULL) {
		...
		if (!is_wp_error($file_content) && $file_content['response']['code'] != '404') {
			if (@simplexml_load_string($file_content['body']) === TRUE) {
				...
				echo sprintf(__('Automatic GPX download cancelled due to security concerns - the file at %1$s does not seem to be a valid XML file!', 'lmm'), '<a style="text-decoration:none;" href="' . $file_url . '">' . $file_url . '</a>');
			}
		} else {
